...
if(!empty($_GET) && $_GET['wsd']=='new'){
	...
	class PDF extends PPDF{
		function Cabecera(){
			global $Desde,$Hasta,$Tipo;
			$this->CuadroCabecera(30,$Tipo." desde: ",30,date("d-m-Y",strtotime($Desde)));
			$this->CuadroCabecera(25,$Tipo." hasta: ",40,date("d-m-Y",strtotime($Hasta)));
			$this->Pagina();	
			$this->ln();
			$this->TituloCabecera(15,"Nº");		
			$this->TituloCabecera(55,"Nombre Completo");
			$this->TituloCabecera(15,"Curso");
			$this->TituloCabecera(30,"Fecha de Pago");
			$this->TituloCabecera(10,"N/C");
			$this->TituloCabecera(20,"Factura");
			$this->TituloCabecera(15,"Monto");
		}
	}
	...
	foreach($cuotas as $cuo){
		$i++;
		$al=$alumno->mostrarDatosCuota($cuo['CodAlumno']);
		$al=array_shift($al);
	...
	$pdf->Fuente("B");
	$pdf->CuadroCuerpo(135,"Total",0,"R",1);

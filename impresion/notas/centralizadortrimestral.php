...
if(!empty($_GET) && isset($_GET['mf']) && $_GET['mf']==md5("lock")){
	...
	$titulo="CENTRALIZADOR DEL TRIMESTRE";
	class PDF extends PPDF{
		...
		function Cabecera(){
			...
			$this->Pagina();
			$this->CuadroCabecera(1,"",30,"R=Reprobados, PR=Promedio del Alumno");
			$this->ln();
	...
	foreach($alumno->mostrarDatosAlumnos($CodCurso) as $al){
		...
		foreach($cursomateria->mostrarMaterias($CodCurso) as $materiasbol){
			$casillas=array_shift($casilleros->mostrarMateriaCursoSexoTrimestre($materiasbol['CodMateria'],$CodCurso,$al['Sexo'],$Trimestre));
			$regNotas=$registronotas->mostrarRegistroNotas($casillas['CodCasilleros'],$al['CodAlumno'],$Trimestre);

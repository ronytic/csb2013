...
if(!empty($_POST)){
	...
	$cur=new curso;
	$rude=new rude;
	$alu=$al->mostrarDatos($CodAlumno);
	$alu=$alu[0];
	$rud=$rude->mostrarDatos($CodAlumno);
	if(count($rud)>=1){
		?>
			YA SEA REGISTRADO EL RUDE DEL ALUMNO: <span class="resaltar"><?php echo mb_strtoupper($alu['Paterno']." ".$alu['Materno']." ".$alu['Nombres'],"utf8");?></span>. <hr />SI QUIERE MODIFICARLO PUEDE IR A LA OPCIÓN MODIFICAR RUDE
		<?php
		exit();	
	}
